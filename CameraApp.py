...
class SettingsWindow(QWidget):
    ...
    def _initFrameViewerLayout(self):
        ...
        frameViewerLayout.setAlignment(Qt.AlignmentFlag.AlignJustify)
...
class FrameViewer(QWidget):
    def __init__(self, camerasAmount: int):
        ...
        self.cameraAttributes: dict[int, dict] = {}
        self.viewWidgets = []
        ...
        self.leftWidget.setStyleSheet(f"background-color: {self.colors['background']}; border-radius: 10px;")
        ...
        # Stacked widget для режимов просмотра
        self.stackedWidget = QStackedWidget()
        ...
        self.allFramesLayout.addWidget(self.scrollArea)
    ...
    def _initViewLayout(self):
        for i in range(self.camerasAmount):
            viewWidget = QWidget()
            viewWidget.setStyleSheet(f"""
                QWidget {{
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 {self.colors['surface']}, stop:1 white);
                    border: 2px solid {self.colors['border']};
                    border-radius: 8px;
                    padding: 10px;
                    margin: 5px;
            ...
            layout = QVBoxLayout(viewWidget)
            layout.setSpacing(8)

            cameraLabel = QLabel(f'📹 Camera {i+1}')
            cameraLabel.setStyleSheet(f"""
                QLabel {{
            ...
            imageLabel.setStyleSheet(f"""
                QLabel {{
                    background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                        stop:0 #e9ecef, stop:1 {self.colors['surface']});
                    border: 2px solid {self.colors['border']};
                    border-radius: 6px;
                    min-height: 180px;
                }}
            """)
            imageLabel.setMinimumSize(300, 180)
            
            no_image = QImage(300, 180, QImage.Format.Format_RGB32)
            no_image.fill(QColor(self.colors['surface']))
            pixmap = QPixmap.fromImage(no_image)
            imageLabel.setPixmap(pixmap)
            layout.addWidget(imageLabel)

            frameInfoLabel = QLabel("📊 Frame: 0/0\n⏰ Time: 0.000s")
            frameInfoLabel.setStyleSheet(f"""
                QLabel {{
            ...
            """)
            frameInfoLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
            layout.addWidget(frameInfoLabel)

            self.scrollLayout.addWidget(viewWidget)
        ...
        self.singleViewWidget = QWidget()
        self.singleViewWidget.setStyleSheet(f"""
            QWidget {{
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 {self.colors['surface']}, stop:1 white);
        ...
        """)
        singleLayout = QVBoxLayout(self.singleViewWidget)
        singleLayout.setSpacing(15)

        self.singleCameraLabel = QLabel('📹 Camera 1 - Full Screen')
        self.singleCameraLabel.setStyleSheet(f"""
            QLabel {{
        ...
        """)
        self.singleCameraLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
        singleLayout.addWidget(self.singleCameraLabel)

        self.singleImageLabel = QLabel()
        self.singleImageLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.singleImageLabel.setStyleSheet(f"""
            QLabel {{
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 #e9ecef, stop:1 {self.colors['surface']});
        ...
        """)
        self.singleImageLabel.setMinimumSize(600, 400)
        
        single_no_image = QImage(600, 400, QImage.Format.Format_RGB32)
        single_no_image.fill(QColor(self.colors['surface']))
        single_pixmap = QPixmap.fromImage(single_no_image)
        self.singleImageLabel.setPixmap(single_pixmap)
        singleLayout.addWidget(self.singleImageLabel)

        self.singleFrameInfoLabel = QLabel("📊 Frame: 0/0 | ⏰ Time: 0.000s | 🎯 Camera: 1")
        self.singleFrameInfoLabel.setStyleSheet(f"""
            QLabel {{
        ...
        """)
        self.singleFrameInfoLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
        singleLayout.addWidget(self.singleFrameInfoLabel)

        self.singleFrameLayout.addWidget(self.singleViewWidget)
    ...
    def _initControlLayout(self):
        ...
        modeLabel = QLabel("🎬 View Mode")
        modeLabel.setStyleSheet(f"font-weight: bold; color: {self.colors['primary']}; font-size: 16px;")
        modeLayout.addWidget(modeLabel)

        self.allCamerasButton = QPushButton("📷 All Cameras")
        self.allCamerasButton.setCheckable(True)
        self.allCamerasButton.setChecked(True)
        self.allCamerasButton.setStyleSheet(self._getModeButtonStyle(True))
        
        modeGroup.addButton(self.allCamerasButton)
        modeLayout.addWidget(self.allCamerasButton)

        self.singleCameraButtons = []
        for i in range(self.camerasAmount):
            btn = QPushButton(f"🎥 Camera {i+1}")
            btn.setCheckable(True)
            btn.setStyleSheet(self._getModeButtonStyle(False))
            modeGroup.addButton(btn)
            modeLayout.addWidget(btn)
            self.singleCameraButtons.append(btn)

        modeGroup.buttonClicked.connect(self._onViewModeChanged)
        mainControlLayout.addLayout(modeLayout)

        mainControlLayout.addSpacing(20)

        navLayout = QVBoxLayout()
        navLabel = QLabel("⏩ Frame Navigation")
        navLabel.setStyleSheet(f"font-weight: bold; color: {self.colors['primary']}; font-size: 16px;")
        navLayout.addWidget(navLabel)

        frameSliderLayout = QVBoxLayout()
        self.frameSlider = QSlider(Qt.Orientation.Horizontal)
        self.frameSlider.setRange(0, 100)
        self.frameSlider.setValue(0)
        self.frameSlider.setStyleSheet(f"""
        ...
        """)
        self.frameSlider.valueChanged.connect(self._onFrameSliderChanged)
        frameSliderLayout.addWidget(self.frameSlider)

        self.frameInfoLabel = QLabel("Frame: 0/0")
        self.frameInfoLabel.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.frameInfoLabel.setStyleSheet(f"color: {self.colors['text_primary']}; font-weight: bold;")
        frameSliderLayout.addWidget(self.frameInfoLabel)
        navLayout.addLayout(frameSliderLayout)

        buttonLayout = QHBoxLayout()
        
        self.prevButton = QPushButton("⏪ Prev")
        self.playPauseButton = QPushButton("⏸️ Pause")
        self.nextButton = QPushButton("Next ⏩")
        
        navButtons = [self.prevButton, self.playPauseButton, self.nextButton]
        for btn in navButtons:
            btn.setStyleSheet(self._getControlButtonStyle())
        ...
        self.prevButton.clicked.connect(self._prevFrame)
        self.playPauseButton.clicked.connect(self._togglePlayPause)
        self.nextButton.clicked.connect(self._nextFrame)
        ...
        playLabel = QLabel("🎛️ Playback Settings")
        playLabel.setStyleSheet(f"font-weight: bold; color: {self.colors['primary']}; font-size: 16px;")
        playLayout.addWidget(playLabel)

        fpsLayout = QVBoxLayout()
        fpsHeaderLayout = QHBoxLayout()
        fpsHeaderLayout.addWidget(QLabel("📊 FPS:"))
        self.fpsValueLabel = QLabel("24")
        self.fpsValueLabel.setStyleSheet(f"color: {self.colors['accent']}; font-weight: bold;")
        fpsHeaderLayout.addWidget(self.fpsValueLabel)
        fpsHeaderLayout.addStretch()
        fpsLayout.addLayout(fpsHeaderLayout)

        self.fpsSlider = QSlider(Qt.Orientation.Horizontal)
        self.fpsSlider.setRange(1, 60)
        self.fpsSlider.setValue(24)
        self.fpsSlider.setStyleSheet(self.frameSlider.styleSheet())
        self.fpsSlider.valueChanged.connect(self._onFpsSliderChanged)
        fpsLayout.addWidget(self.fpsSlider)

        playLayout.addLayout(fpsLayout)
        mainControlLayout.addLayout(playLayout)

        mainControlLayout.addStretch()

    def _getModeButtonStyle(self, is_checked):
        base_style = f"""
            QPushButton {{
                background-color: {self.colors['surface']};
                color: {self.colors['text_primary']};
    ...
    def _getControlButtonStyle(self):
        return f"""
            QPushButton {{
                background-color: {self.colors['primary']};
                color: white;
    ...
    def _onViewModeChanged(self, button):
        for btn in self.singleCameraButtons:
            btn.setStyleSheet(self._getModeButtonStyle(False))
        self.allCamerasButton.setStyleSheet(self._getModeButtonStyle(False))

        button.setStyleSheet(self._getModeButtonStyle(True))

        if button == self.allCamerasButton:
            self.stackedWidget.setCurrentIndex(0)
            self.cameraSelectedIndex = -1
        else:
            self.stackedWidget.setCurrentIndex(1)
            self.cameraSelectedIndex = self.singleCameraButtons.index(button)
            self._updateSingleView()

    def _onFrameSliderChanged(self, value):
        if self.maxFrames > 0:
            self.currentFrameIndex = min(value, self.maxFrames - 1)
            self._updateDisplay()

    def _onFpsSliderChanged(self, value):
        self.fpsValueLabel.setText(str(value))
        if self.isPlaying:
            self.playTimer.setInterval(1000 // value)

    def _prevFrame(self):
        if self.maxFrames > 0:
            self.currentFrameIndex = (self.currentFrameIndex - 1) % self.maxFrames
            self._updateDisplay()
    ...
    def _togglePlayPause(self):
        ...
        if self.isPlaying:
            self.playPauseButton.setText("⏸️ Pause")
            interval = 1000 // self.fpsSlider.value()
            self.playTimer.start(interval)
        else:
            self.playPauseButton.setText("▶️ Play")
            self.playTimer.stop()
    ...
    def _playNextFrame(self):
        ...
        self.currentFrameIndex = (self.currentFrameIndex + self.playDirection) % self.maxFrames
        self._updateDisplay()

    def _updateDisplay(self):
        self.frameSlider.setValue(self.currentFrameIndex)
        self.frameInfoLabel.setText(f"Frame: {self.currentFrameIndex + 1}/{self.maxFrames}")

        if self.cameraSelectedIndex == -1:
            self._updateAllViews()
        else:
            self._updateSingleView()

    def _updateAllViews(self):
        for i in range(self.camerasAmount):
            if i in self.cameraAttributes:
                data = self.cameraAttributes[i]
                images = data.get('images', [])
                timestamps = data.get('timestamps', [])
                
                if images and self.currentFrameIndex < len(images):
                    pixmap = QPixmap.fromImage(images[self.currentFrameIndex])
                    scaled_pixmap = pixmap.scaled(
                        self.viewWidgets[i]['imageLabel'].size(),
                        Qt.AspectRatioMode.KeepAspectRatio,
                        Qt.TransformationMode.SmoothTransformation
                    )
                    self.viewWidgets[i]['imageLabel'].setPixmap(scaled_pixmap)
                    
                    timestamp = timestamps[self.currentFrameIndex] if self.currentFrameIndex < len(timestamps) else 0
                    self.viewWidgets[i]['frameInfoLabel'].setText(
                        f"📊 Frame: {self.currentFrameIndex + 1}/{len(images)}\n"
                        f"⏰ Time: {timestamp:.3f}s"
                    )

    def _updateSingleView(self):
        if self.cameraSelectedIndex in self.cameraAttributes:
            data = self.cameraAttributes[self.cameraSelectedIndex]
            images = data.get('images', [])
            timestamps = data.get('timestamps', [])
            
            if images and self.currentFrameIndex < len(images):
                self.singleCameraLabel.setText(f'📹 Camera {self.cameraSelectedIndex + 1} - Full Screen')
                
                pixmap = QPixmap.fromImage(images[self.currentFrameIndex])
                scaled_pixmap = pixmap.scaled(
                    self.singleImageLabel.size(),
                    Qt.AspectRatioMode.KeepAspectRatio,
                    Qt.TransformationMode.SmoothTransformation
                )
                self.singleImageLabel.setPixmap(scaled_pixmap)
                
                timestamp = timestamps[self.currentFrameIndex] if self.currentFrameIndex < len(timestamps) else 0
                self.singleFrameInfoLabel.setText(
                    f"📊 Frame: {self.currentFrameIndex + 1}/{len(images)} | "
                    f"⏰ Time: {timestamp:.3f}s | "
                    f"🎯 Camera: {self.cameraSelectedIndex + 1}"
                )

    def loadData(self, cameraIndex, images, timestamps):
        self.cameraAttributes[cameraIndex] = {
            'images': images,
            'timestamps': timestamps
        }
        
        frame_counts = [len(data.get('images', [])) for data in self.cameraAttributes.values()]
        self.maxFrames = max(frame_counts) if frame_counts else 0
        
        self.frameSlider.setRange(0, max(0, self.maxFrames - 1))
        self.frameInfoLabel.setText(f"Frame: 1/{self.maxFrames}")
